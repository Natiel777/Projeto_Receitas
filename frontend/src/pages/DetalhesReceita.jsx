...
function DetalhesReceita() {
  ...
  const media = avaliacoes.length
    ? (avaliacoes.reduce((s, a) => s + a.nota, 0) / avaliacoes.length).toFixed(
        1
      )
    : "Sem avaliações";
    ...
    <div className="max-w-3xl w-full mx-auto px-4 sm:px-6 py-6 font-sans">
      ...
      {autenticado && (
        <form onSubmit={enviarComentario} className="mt-6 space-y-4">
          <textarea
            className="w-full border rounded-xl p-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition"
