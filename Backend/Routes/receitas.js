...
router.post('/publicar', auth, upload.single('imagem'), async (req, res) => {
  ...
  const { lastID } = await db.run(
    'INSERT INTO receitas (titulo, ingredientes, modo_preparo, imagem, autor_id) VALUES (?, ?, ?, ?, ?)',
    [titulo, ingredientes, modo_preparo, req.file.filename, req.user.id]
  );
...
router.put('/:id', auth, async (req, res) => {
  ...
  await db.run(
    ...
    [titulo, ingredientes, modo_preparo, req.params.id]
  );
